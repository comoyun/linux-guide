...
Kompyuterning ommaviy IP (Internet Protocol) manzilini aniqlash uchun buyruq:
...
So'ng, ssh xizmatini yoqishimiz mumkin (bilaman, `service` buyrug'i eski, lekin bu barcha Linux distributivlarida ishlaydi.):

```bash
service ssh start
```

status ya'ni xolatini aniqlash uchun:
