...
Jadvalni yodlash maqsadga muvofiq bo'ladi:
...
O'zgaruvchilarni ko'rsatish (muhit - Google Cloud Shell):
...
Har bir buyruq chiqish kodini qaytaradi. Muvaffaqiyatli buyruq '0' ni qaytaradi, muvaffaqiyatsiz bo'lsa, 1 dan 255 gacha bo'lgan qiymatni qaytaradi. 

```bash
$ exit 0 # muvaffaqiyatli
$ exit 1 # umumiy xatolikni bildiradi
```

Dastur tugagach, chiqish kodi `$?` o'zgaruvchisiga saqlanadi. Dastur muvaffaqiyatli bajarilganini tekshirish uchun: `echo $?`.
...
echo $REPLY # kiritilgan ma'lumotni chop etadi
```
